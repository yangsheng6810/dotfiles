;;; evil-bind-smartparens.el --- Evil bindings to Smartparens
;;; original name evil-smartparens.el --- Evil bindings to Smartparens
...
(define-minor-mode evil-bind-smartparens-mode
  "Minor mode for combining Evil and SmartParens."
  :keymap '()
  :lighter "E-SP")

(defun evil-bind-sp-evilize-name (name)
  "Return an interned symbol NAME prefixed with 'evil-'."
   (intern (format "evil-%s" name)))

(defmacro evil-bind-sp-make-evil-smartparens-movement (name)
  "Create Evil movement command from NAME function."
  `(let ((evil-name (evil-bind-sp-evilize-name ,name))
...
(defun evil-bind-sp-beginning-of-previous-sexp (&optional arg)
...
(defun evil-bind-sp-smartparens-config ()
  ...
  (let ((sexp-motions '(("zh" . sp-backward-sexp)
                        ...
                        ("zp" . evil-bind-sp-beginning-of-previous-sexp)
                        ("zJ" . sp-end-of-sexp)
        ...
        (sexp-modifications '(("zst" . sp-transpose-sexp)
                              ...
                              ("zsy" . sp-split-sexp)
                              ("zsY" . sp-join-sexp))))
    (unbind-key "zf" evil-normal-state-map)

    (loop for (key . func) in sexp-motions
          do
          ;; Define the motion command
          (evil-bind-sp-make-evil-smartparens-movement func)
...
(evil-bind-sp-smartparens-config)

(provide 'evil-bind-smartparens)
;;; evil-bind-smartparens.el ends here
