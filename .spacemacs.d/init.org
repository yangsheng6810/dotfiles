...
(when yang/at-china
  ...
  (setq socks-server '("Default server" "127.0.0.1" 8889 5))
  (setq request--url-options
        '("--proxy" "socks5://localhost:8889"))
...
(define-advice gcmh-idle-garbage-collect (:around (func) yang)
  "Hide garbage collect message in *messages*."
...
(when (display-graphic-p)
  ...
  (let* ((my-font (font-xlfd-name
                   (font-spec :family "Sarasa Mono SC" :weight 'normal
    ...
    (set-fontset-font my-fontset 'symbol "Noto Color Emoji" nil 'append)
  ...
  (let* ((fontset-name "variable")
        (my-font (font-xlfd-name
                  (font-spec :family "Bookerly" :weight 'normal
                             :slant 'normal :width 'normal)))
        (my-fontset (concat "fontset-" fontset-name)))
    (create-fontset-from-ascii-font my-font nil fontset-name)
  ...
  (let* ((fontset-name "monoserif")
        (my-font (font-xlfd-name
                  (font-spec :family "Latin Modern Mono" :weight 'normal
                             :slant 'normal :width 'normal)))
        (my-fontset (concat "fontset-" fontset-name)))
...
(use-package org-agenda
  ...
  (add-hook 'org-agenda-mode-hook #'yang/fixed-pitch-setup)
  (org-super-agenda-mode)

  ;; Copied from [[https://emacs.stackexchange.com/a/60616][Share]]
  (defun jarfar/org-agenda-cmp-user-defined-created-date (a b)
    ...
    (let* ((def (if org-agenda-sort-notime-is-late 99999999 -1))
           ...
           (time-a (if marker-a (org-entry-get marker-a "CREATED") def))
           (time-b (if marker-b (org-entry-get marker-b "CREATED") def)))
   ...
   '(
     ...
     ("gc" "Computer" tags-todo "computer|linux|emacs"
      ((org-agenda-skip-function '(org-agenda-skip-entry-if 'scheduled 'deadline))
       ...
       (org-agenda-sorting-strategy '(priority-down category-keep user-defined-down))
  ...
  ;; Deal with the following bug: condition-case: Error in a Doom startup hook:
  ;; org-agenda-finalize-hook, elegant-agenda--finalize-view, (void-function
  ;; \(setf\ buffer-string\)) ;;
...
(defun yang/set-org-capture-templates (&rest _)
  (interactive)
  (message "YANG: setting up org-capture-templates")
         ...
         '(
           ...
           ("Schedule" :keys "s" :file "~/Documents/org/Schedule.org"
            ...
            :template ("* %^{Description}"
                       ...
                       ":END:"
                       "%a%?"))
           ("Schedule (work)" :keys "S" :file "~/Documents/org/Schedule.org"
            :datetree t
            :template ("* %^{Description}"
                       ":PROPERTIES:"
                       ":Created: %U"
           ...
           ("Tasks" :keys "t" :file "~/Documents/org/Tasks.org"
            ...
            (("Computer"
             ...
             ("Learn"
              :keys "l" :headline "Learn" :todo-state "TODO")
             ("Food"
           ...
           ("Protocol Link" :keys "L" :file "~/Documents/org/Notes.org"
            ...
            :template ("* %{todo-state} [[%:link][%(transform-square-brackets-to-round-ones \"%:description\")]]"
                       ...
                       "%?")
            :todo-state "TODO"))))
  (advice-remove 'org-capture #'yang/set-org-capture-templates))
(define-advice +org-init-capture-defaults-h (:after () yang)
  (message "YANG: advice in setting up org-capture-templates")
  (advice-add 'org-capture :before #'yang/set-org-capture-templates))
...
** org-latex-instant-preview
   #+begin_src emacs-lisp
(when (-contains? '("Desktop" "carbon" "yoga")
                  (system-name))
  (use-package org-latex-impatient
    ...
    (setq org-latex-impatient-tex2svg-bin
          ...
          (if (equal (system-name) "yoga")
              "~/.node_modules/bin/tex2svg"
...
(use-package org-special-block-extras
  ...
  :config
  (general-def
    :keymaps 'org-special-block-extras-mode-map
    "C-h" nil))
#+end_src
* LaTeX
  #+BEGIN_SRC emacs-lisp
...
** Fix xparse
#+begin_src emacs-lisp
(with-eval-after-load 'xparse
  (define-advice LaTeX-xparse-macro-parse (:override (type) yang)
...
TYPE is one of the symbols mac or env."
    (dolist (xcmd (if (eq type 'mac)
      ...
      ;; (message "xcmd is %S" xcmd)
      ;; (message "xcmd type is %s" (type-of xcmd))
      (when (nth 2 xcmd)
        (let ((name (nth 1 xcmd))
              (spec (nth 2 xcmd))
              (what (nth 3 xcmd))
...
(use-package ivy
  ...
  (dolist (func '(org-roam-node-insert
                  org-roam-node-find
                  swiper-isearch))
    (add-to-list 'ivy-re-builders-alist
                 `(,func . ivy--cregex-plus))))
...
(use-package ace-pinyin
  ...
  :commands (ace-pinyin-jump-char-2
             ace-pinyin-goto-char-timer)
  :init
  (yang/set-leader-key "jj" #'ace-pinyin-jump-char-2)
  (yang/set-leader-key "jJ" #'ace-pinyin-goto-char-timer)
  :config
...
(use-package ivy
  ...
  (add-to-list 'ivy-re-builders-alist '(swiper-isearch . ivy--cregex-plus))
  (add-to-list 'ivy-re-builders-alist '(org-roam-node-find . ivy--cregex-plus))
  ;; (setq ivy-re-builders-alist
  ;;       '(
  ;;         ;; (t . re-builder-pinyin)
  ;;         (t . ivy--cregex-plus)
...
(use-package elfeed
  ...
  (setq +rss-workspace-name "elf")
  (setf elfeed-curl-extra-arguments '("--socks5-hostname" "localhost:8889"))

  (define-advice elfeed-db-compact (:around (func ) yang)
    (cl-letf (((symbol-function 'delete-file-projectile-remove-from-cache) 'ignore))
      (funcall func)))
                ...
                (run-with-idle-timer
                 ...
                 (lambda ()
                   (unless (get-buffer "*elfeed-search")
                     ...
                     (message "Compacting Elfeed Database in spare time, please wait...")
...
(use-package magit
  ...
  (defun yadm-fix-env (&rest _)
    ...
    (advice-remove #'+vterm/toggle #'yadm-fix-env)
  ...
  (defun yadm-magit-status ()
    ...
    (let ((magit-mode-hook
      ...
      (advice-add #'+vterm/toggle :before #'yadm-fix-env)
...
(use-package ledger-mode
  ...
  (ledger-reports
   '(("credit card balance" "%(binary) [[ledger-mode-flags]] -f /home/yangsheng/Documents/Finance/mine.ledger bal Liabilities")
     ("net worth" "%(binary) [[ledger-mode-flags]] -f /home/yangsheng/Documents/Finance/mine.ledger bal ^Assets:BofA ^Assets:Ally ^Assets:Cash ^Assets:Fidelity ^Assets:BoC ^Liabilities -C -R")
...
(defun yang/switch-to-prev-buffer-skip (window buffer bury-or-kill)
  (not (derived-mode-p 'helpful-mode)))

(defun yang/unset-dedicate-window-temporarily (func &rest args)
  (if (derived-mode-p 'helpful-mode)
      (progn
        (set-window-dedicated-p (selected-window) nil)
        (let ((switch-to-prev-buffer-skip #'yang/switch-to-prev-buffer-skip))
          (apply func args))
        (when (derived-mode-p 'helpful-mode)
          (set-window-dedicated-p (selected-window) 'popup)))
    (apply func args)))

(advice-add #'previous-buffer :around  #'yang/unset-dedicate-window-temporarily)
(advice-add #'next-buffer :around  #'yang/unset-dedicate-window-temporarily)
  #+END_SRC
...
(use-package telega
  ...
  :config
  (setq telega-proxies (list '(:server "127.0.0.1" :port "8889" :enable t :type (:@type "proxyTypeSocks5"))))
  ...
  (setq telega-known-inline-bots '("@gif" "@youtube" "@pic"
                                   ...
                                   "@ReadabBot" "@nixcaosaysbot" "@TwPicBot"))
...
(use-package emms
  :defer t
  :commands (yang/start-emms yang/evil-collection-emms-mark-setup)
  :config
  
  (defun yang/evil-collection-emms-mark-setup ()
    (interactive)
    (evil-set-initial-state 'emms-mark-mode 'normal)
    (evil-collection-define-key 'normal 'emms-mark-mode-map
      "D"         'emms-mark-delete-marked-tracks
      "K"         'emms-mark-kill-marked-tracks
      "U"         'emms-mark-unmark-all
      "C"         'emms-mark-copy-marked-tracks
      "m"         'emms-mark-forward
      "t"         'emms-mark-toggle
      "u"         'emms-mark-unmark-forward
      "% m"       'emms-mark-regexp))

  (defun yang/emms-switch-to-current-playlist ()
    (interactive)
    (setq emms-playlist-buffer (current-buffer)))

  (defun yang/start-emms ()
    (interactive)
    ;; (setq emms-setup-default-player-list
    ;;       '(emms-player-mpv emms-player-mpg321 emms-player-ogg123 emms-player-mplayer-playlist emms-player-mplayer emms-player-vlc emms-player-vlc-playlist))
    (setq emms-setup-default-player-list
          '(emms-player-mpv))
    (setq emms-source-file-default-directory "/mnt/HPE/Data/音乐/")
    ...
    (emms)
    (yang/evil-collection-emms-mark-setup)
    (custom-set-faces
   '(emms-playlist-selected-face ((t (:inherit org-date-selected))))
   '(emms-playlist-track-face ((t (:inherit default))))))

  (define-advice emms-playlist-new (:override  (&optional name) yang)
...
** Image open
#+begin_src emacs-lisp
(defun yang/img-reopen ()
      (interactive)
      (if (buffer-file-name)
          (call-process-shell-command (concat "feh --zoom 200 " (prin1-to-string (buffer-file-name))) nil 0 nil)
        (message "No file available")))
#+end_src

** repeat-prefix
from: https://karthinks.com/software/persistent-prefix-keymaps-in-emacs/
#+begin_src emacs-lisp
...
(setq prefix-help-command/backup prefix-help-command)
