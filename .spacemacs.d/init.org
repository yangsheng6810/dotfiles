       ...
       (use-package org-gcal
         ...
         (defun org-gcal-capture-before-func ()
           (let ((buffer-name (prin1-to-string (current-buffer))))
             (when (string-match-p "CAPTURE-gcal.org" buffer-name)
...
** notmuch
   #+BEGIN_SRC emacs-lisp
         ...
         (let ((key-string (concat prefix (key-description actual-key))))
           ...
           (unless (assoc key-string tail)
             ...
             (push (cons key-string
                   ...
                   tail)))
         tail)
     (defun notmuch-save-attachments-for-export (mm-handle directory)
     ...
     (defun notmuch/mu4e-remove-file-later (filename)
       "Remove FILENAME in a few seconds."
       (lexical-let ((filename filename))
         (run-at-time "30 sec" nil
                      (lambda () (ignore-errors (delete-file filename))))))

     (defun notmuch/mu4e-make-temp-file (ext)
     ...
     program."
       (let ((tmpfile (make-temp-file "mu4e-" nil (concat "." ext))))
         (notmuch/mu4e-remove-file-later tmpfile)
         tmpfile))

     (defun yang/notmuch-write-body-to-html ()
       "Write the body (either html or text) to a temporary file;
     return the filename."
       (interactive)
       (with-current-notmuch-show-message
        (let ((mm-handle (mm-dissect-buffer))
              (tmpfile (notmuch/mu4e-make-temp-file "html"))
              html
              txt
         ...
         (with-temp-buffer
           ...
           (notmuch-save-attachments-for-export
