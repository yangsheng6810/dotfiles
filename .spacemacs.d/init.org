...
* Display
  #+BEGIN_SRC emacs-lisp
    ...
    (use-package beacon
      ...
      (defun yang/beacon--space-pixel-width ()
        (save-excursion
          (let* ((old-pixel-x (car (nth 2 (posn-at-point))))
                 (new-pixel-x)
                 (temp-ov (make-overlay (point) (point))))
            (overlay-put temp-ov 'after-string " ")
            (setq new-pixel-x (car (nth 2 (posn-at-point))))
            (delete-overlay temp-ov)
        (if (= new-pixel-x old-pixel-x)
            (window-font-width)
            (- new-pixel-x old-pixel-x)))))

      (defun yang/beacon--visual-current-column (func &rest args)
        (interactive)
        (if (not (and (display-graphic-p)
                      (boundp 'buffer-face-mode-face)
                      (eq buffer-face-mode-face 'variable-pitch)))
            (apply func args)
          (let* ((cur-pos (posn-at-point))
                 (cur-char-pixel-width (car (nth 9 cur-pos)))
                 (cur-x (car (nth 2 cur-pos)))
                 (window-pixel-width (window-body-width nil t))
                 (space-pixel-width (yang/beacon--space-pixel-width)))
            (- (window-width) (/ (- window-pixel-width cur-x cur-char-pixel-width) space-pixel-width) 1))))
      :config
      ;; (advice-add #'beacon--visual-current-column :around #'yang/beacon--visual-current-column)
