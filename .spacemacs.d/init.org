...
** GC
   #+begin_src emacs-lisp
(setq gcmh-high-cons-threshold (* 160 1024 1024) ; 160MB
      gcmh-idle-delay 15)
   #+end_src
...
If IGNORE-REGION-P is non-nil, only return the entry under point."
  ...
  ;; do not compact elfeed-db, as it is very slow
  (define-advice +rss-cleanup-h (:around (func) yang)
    (interactive)
    (define-advice elfeed-db-compact (:override () yang) nil)
    (funcall func)
    (advice-remove 'elfeed-db-compact #'elfeed-db-compact@yang)
    (let ((buf (get-buffer "*elfeed-search*")))
      (when buf
        (kill-buffer buf)))
...
(use-package telega
  ...
  (setq telega-chat-button-width 25)
  (setq telega-root-fill-column (+ 40 telega-chat-button-width))
